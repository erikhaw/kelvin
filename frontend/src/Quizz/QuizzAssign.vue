...
const teacherNames = ref<string[]>([...new Set(assignmentsData.value.map(c => c.teacher)), 'All']);
...
const setToAllClasses = async (target: AssignmentClass) => {
  ...
  properties.forEach(property => {
    assignmentsData.value.filter(c => c.visible).forEach(clazz => {
 ...
 */
const setToAllClassesByProperty = async (target: AssignmentClass, property: AssignmentClassProperty) => {
    ...
    <div class="card">
      ...
      <div :class="{'collapse': isCollapsed, 'show': !isCollapsed}">
        <div class="d-flex justify-content-end m-1">
          <button class="btn btn-secondary" @click="setPublishToAllClasses">Publish all results</button>
        ...
        <table id="assign" class="table m-0">
          <tbody>
          <tr class="col-12 p-2" v-for="clazz in assignmentsData" :key="clazz.id"
          ...
          >
            ...
            <td>
              <div class="row col-12">
                <div class="col-lg d-flex justify-content-start">
                  <VueDatePicker class="border-0" placeholder="Active from" v-model="clazz.assigned"/>
                  <button
                ...
                <div class="col-lg d-flex justify-content-start">
                  <VueDatePicker :disabled="!clazz.assigned" placeholder="Deadline"
            ...
            <td>
              <div class="form-check d-flex p-0">
                ...
                />
                <label :for="'publish-results-input-' + clazz.id" class="form-check-label">&nbsp;Publish
                  results</label>
              </div>
            </td>
            <td>
